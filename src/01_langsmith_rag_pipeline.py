...
RAG_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "Nếu context không chứa thông tin, hãy nói rõ là bạn không tìm thấy thông tin.\n\n"
     "Context:\n{context}"),
    ("human", "{question}"),
])
...
def build_rag_chain(vectorstore):
    """
    Xây dựng LCEL RAG chain: retriever → prompt → LLM → StrOutputParser.

    Args:
        vectorstore: FAISS vectorstore đã index.

    Returns:
        (chain, retriever) — chain nhận vào câu hỏi dạng str và trả về str.
    """
    llm       = get_llm()
